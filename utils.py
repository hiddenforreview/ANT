...
def write_performance(header, csv_filename, performance):
    ...
    row = ['   subset    :', round(performance[0],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   hamming   :', round(performance[1],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   preMI     :', round(performance[2],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   recallMI  :', round(performance[3],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   f1MI      :', round(performance[4],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   preMA     :', round(performance[5],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   recallMA  :', round(performance[6],4)]; write_row(row, csv_filename, path=logpath, round=False)
    row = ['   f1MA      :', round(performance[7],4)]; write_row(row, csv_filename, path=logpath, round=False)
